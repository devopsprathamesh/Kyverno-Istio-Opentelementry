...
- **Phase 3 reconciliation:** `kyverno/` implements this exactly as planned above, with one addition — its automation (`kyverno/scripts/lib/kubernetes.sh`) deliberately discovers webhook configuration names by prefix (`any_kyverno_webhook_exists`) rather than hardcoding an exact name, since exact webhook-config and some CRD names are Kyverno chart-version-dependent implementation details (see `kyverno/docs/02-architecture-and-internals.md`'s note on this). `kyverno/install/values-*.yaml` extend (not replace) the chart's own default `config.resourceFilters` exclusion list via `config.resourceFiltersExcludeNamespaces`, adding `kube-system`/`kube-public`/`kube-node-lease`/`cilium`/`hubble` on top of Kyverno's own built-in defaults (which already exclude its own namespace via `excludeKyvernoNamespace`).
...
- Cilium and Istio can coexist: Cilium provides the CNI/eBPF datapath and NetworkPolicy enforcement at L3/L4, while Istio's sidecars provide L7 traffic management, mTLS, and authorization on top. They are not mutually exclusive; this repository runs them together starting in the independent Istio lab (Phase 4), and again alongside Kyverno/observability in the integrated lab (Phase 6).
- **Known area of friction:** Istio sidecar mode relies on iptables (or, in some configurations, a CNI plugin) to redirect pod traffic into the Envoy proxy. When Cilium is also managing pod networking, both are intercepting/redirecting traffic on the same pods. The Istio CNI plugin mode (as opposed to the init-container iptables approach) is the safer default when Cilium is present, since it avoids double-managing iptables rules — this is the mode `istio/` (Phase 4) exclusively uses, via **CNI chaining** rather than either CNI replacing the other.
- **Confirmed, specific gap found in Phase 4 (not hypothetical):** CNI chaining requires Cilium's own Helm values to cooperate — specifically `cni.exclusive: false` (Cilium's own default is `true`, which assumes it is the only CNI plugin) and `socketLB.hostNamespaceOnly: true`. This repository's Phase 2 Cilium install (`auto-setup-default-kube-env/`) does **not** set either value — they weren't needed for a Cilium-only cluster, and Phase 2 correctly didn't anticipate a future Istio phase. `istio/scripts/verify-cluster.sh` detects this (warns) and `istio/scripts/install.sh` hard-fails on it immediately before installing the Istio CNI plugin, printing the exact one-line `helm upgrade --reuse-values` remediation. Per this repository's module-isolation rules, `istio/` never runs that remediation itself — it is a manual, disclosed step against the existing Cilium release. Full detail: [`../istio/docs/04-istio-cni-and-cilium.md`](../istio/docs/04-istio-cni-and-cilium.md).
- This is called out as a specific integrated-lab risk (deeper multi-layer interaction, e.g. Cilium NetworkPolicy exercised alongside Istio) in [`PROJECT-IMPLEMENTATION-PLAN.md`](../PROJECT-IMPLEMENTATION-PLAN.md) Phase 6; the CNI-chaining compatibility itself is validated in Phase 4's own `tests/cilium-compatibility-test.sh` (runtime, pending — see [`VALIDATION-STATUS.md`](VALIDATION-STATUS.md)).
